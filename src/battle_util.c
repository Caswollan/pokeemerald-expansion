    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_DEMORALISE:
            if (shouldAbilityTrigger && !IsOpposingSideEmpty(battler))
            {
                ...
                BattleScriptCall(BattleScript_DemoraliseActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_SPICY_SPRAY:
             ...
             && CanBeBurned(gBattlerTarget, gBattlerAttacker, GetBattlerAbility(gBattlerAttacker)))
            {
                ...
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptCall(BattleScript_SpicySprayBurn);
                effect++;
            }
            break;
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if (ctx->moveType == TYPE_POISON && defType == TYPE_STEEL && ctx->abilityAtk == ABILITY_CORROSION && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilityAtk);
    }
    else if (gMovesInfo[ctx->move].boneMove && ctx->abilityAtk == ABILITY_BONE_ZONE && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilityAtk);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_POISON && ctx->abilityDef == ABILITY_PASTEL_VEIL)
    {
        modifier = UQ_4_12(0.0);
