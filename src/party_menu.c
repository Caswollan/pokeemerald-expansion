...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    ...
    if ((GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_RareCandy && CheckBagHasItem(gSpecialVar_ItemId, 1) && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD)
        || (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_InfiniteCandy && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD))
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        ...
        else if (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_InfiniteCandy && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD)
    ...
    {
        if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && (CheckBagHasItem(gSpecialVar_ItemId, 1) || GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_InfiniteCandy))
            gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
        else
            gTasks[taskId].func = Task_ClosePartyMenuAfterText;
    }
